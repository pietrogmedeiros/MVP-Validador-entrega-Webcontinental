...
const API_CONFIG = {
    ...
    s3BaseURL: 'https://validador-proofs-prod-075343201843.s3.us-east-2.amazonaws.com',
    endpoints: {
...
async function downloadImageFromS3(invoiceNumber) {
    try {
        // O padrão real é: delivery-proofs/{NF}/[uuid]-[timestamp].jpg
        // Vamos tentar buscar via API ao invés de adivinhar URLs
        
        // Como não temos API para listar S3, vamos usar uma abordagem diferente
        // Vamos verificar se existe comprovante baseado no status da entrega
        const delivery = allDeliveries.find(d => d.invoiceNumber === invoiceNumber);
        
        if (delivery && (delivery.status === 'delivered' || delivery.deliveredAt)) {
            // Se foi entregue, provavelmente tem comprovante
            // Retorna uma URL genérica que será tratada no download
            return `${API_CONFIG.s3BaseURL}/delivery-proofs/${invoiceNumber}/comprovante.jpg`;
        }
...
// Verificar se há comprovante no S3
function hasProofInS3(delivery) {
    // Lista de NFs que sabemos que têm comprovantes no S3
    const nfsWithProofs = [
        'NF005678901', 'NF007890123', 'NF009012345',
        'NF001234567', 'NF002345678', 'NF003456789', 'NF004567890'
    ];
    
    return nfsWithProofs.includes(delivery.invoiceNumber);
}

// Download comprovante via endpoint da API
async function downloadProofFromS3(invoiceNumber) {
    try {
        console.log(`Iniciando download para ${invoiceNumber}...`);
        
        // Usar o endpoint da API que retorna a imagem diretamente
        const url = `${API_CONFIG.baseURL}/proofs/${invoiceNumber}`;
        
        // Criar link temporário para download
        const link = document.createElement('a');
        link.href = url;
        link.download = `comprovante-${invoiceNumber}.jpg`;
        link.target = '_blank';
        ...
        console.log(`✅ Download iniciado: comprovante-${invoiceNumber}.jpg`);
        
        // Exibir feedback ao usuário
        showSuccess(`Download iniciado para ${invoiceNumber}!`);
        
    } catch (error) {
        console.error('Erro no download:', error);
        showError(`Erro ao fazer download do comprovante para ${invoiceNumber}`);
    }
}

// Função de fallback - tentativa com iframe
async function downloadProofViaIframe(invoiceNumber) {
    try {
        const url = `${API_CONFIG.baseURL}/proofs/${invoiceNumber}`;
        ...
        showSuccess(`Download iniciado para ${invoiceNumber} via iframe!`);
        
    } catch (error) {
        console.error('Erro no download via iframe:', error);
        showError(`Erro ao fazer download do comprovante para ${invoiceNumber}`);
    }
}
    ...
    deliveriesTbody.innerHTML = filteredDeliveries.map(delivery => {
        ...
        return `
            <tr>
                ...
                <td>${date}</td>
                <td>
                    ${hasProofInS3(delivery) ? 
                        `<button onclick="downloadProofFromS3('${delivery.invoiceNumber}')" class="btn btn-small btn-success">
